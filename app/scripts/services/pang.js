angular.module('pang', []).factory('pang', function($rootScope, $timeout) {
  ...
  *******************************************************************/
  var parse2pangObject = function(parseObject) {
    ...
    object.createdAt = parseObject.createdAt.toISOString();
    if (parseObject.dispatchedAt) {
      object.dispatchedAt = parseObject.dispatchedAt.toISOString();
    }
    if (parseObject.deliveredAt) {
      object.deliveredAt = parseObject.deliveredAt.toISOString();
    }
    if (parseObject.closedAt) {
      object.closedAt = parseObject.closedAt.toISOString();
    }
    if (parseObject.canceledAt) {
      object.canceledAt = parseObject.canceledAt.toISOString();
    }
        ...
        success: function(parseObject) {
          for(var attrKey in pangObject) {
            ...
            //otherwise regular key
              ...
              // unset keys instead
              if (pangObject[attrKey] == '') {
                parseObject.unset(attrKey);
              } else {
